...
#import "PCRecordEncoder.h"

@interface PCRecordManage ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate> {
...
@property (strong, nonatomic) PCRecordEncoder            *recordEncoder;//录制编码
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.maxRecordTime = VidoeMaxTime;
    }
    return self;
}
...
- (AVCaptureSession *)recordSession {
    if (_recordSession == nil) {
        ...
        AVCaptureConnection *captureConnection = [self.imageOutput connectionWithMediaType:AVMediaTypeAudio];
...
- (void)starCameraStillImage
{
    ...
    id stillImageBlock = ^(CMSampleBufferRef sampleBuffer,NSError *error){
        __strong typeof(self) strongSelf = weakSelf;
        [strongSelf stillImageBlock:sampleBuffer];
    };
    [self.imageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:stillImageBlock];
}
#pragma mark 写入本地照片
- (void)stillImageBlock:(CMSampleBufferRef)sampleBuffer{
    [self.delegate captureStillImageSuccess:sampleBuffer];
}
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [PCRecordEncoder encoderForPath:self.videoPath Height:_cy width:_cx channels:_channels samples:_samplerate];
        }
    ...
    NSLog(@"当前录制时间-----%f",self.currentRecordTime);
    // 进行数据编码
